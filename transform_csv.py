...
from CRUD.equipment_repo import create_equipment, add_exercise_to_equipment
...
def transform_equipment_csv():
    ...
    #  #     instructions = data[i]['instructions'][2:-2]  #  #     new_exercise = Exercise(  #         force=data[i]['force'].upper(),  #         name=data[i]['name'],  #         instructions=instructions,  #         level=data[i]['level'].upper(),  #         mechanic=data[i]['mechanic'],  #         avg_time_per_set=43  #     )  #  #     # Call the create_exercise method to add the new exercise to the session  #     ex_name = create_exercise(new_exercise)  #  #     primary_muscle_to_add = data[i]['primaryMuscles'][2:-2]  #     add_primary_muscle_to_exercise(primary_muscle_to_add, ex_name)  #  #     secondary_muscles = data[i]['secondaryMuscles'][2:-2].split(',')  #     for sec_muscle in secondary_muscles:  #         # Clean string of all punctuation and whitespace  #         sec_muscle = sec_muscle.strip().replace("'", "").replace('"', "").replace('[', '').replace(']', '')  #         add_secondary_muscle(sec_muscle, ex_name)  #     print(data[i]['secondaryMuscles'])
